...
(function (global) {
  ...
  function renderContent(content, doc) {
    ...
    if (content.hero.video) {
      ...
      heroSlot.innerHTML = `<video class="dash-media" autoplay muted loop playsinline${posterAttr}><source src="${escapeHtml(content.hero.video)}">Your browser doesn't support embedded video.</video>`;
    } else if (content.hero.image) {
      heroSlot.innerHTML = `<img class="dash-media" src="${content.hero.image}" alt="Project dashboard preview">`;
    } else {
