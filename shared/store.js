...
(function (global) {
  ...
  // ---------------------------------------------------------
  // PUBLIC-SITE CONTENT LOADING
  // Used only by the public page (js/main.js), never the Admin
  // Panel. Priority order:
  //   1. localStorage on THIS browser/origin, if present — this
  //      is what makes "edit in Admin Panel -> see it instantly
  //      on localhost" keep working exactly as before.
  //   2. content.json shipped alongside the site — this is what
  //      real visitors on the published GitHub Pages URL see,
  //      since their browser has no localStorage for your site.
  //   3. DEFAULT_CONTENT, if neither of the above is available
  //      (e.g. first run, or content.json fails to load because
  //      the page was opened as a local file:// instead of via
  //      a server).
  ...
  function loadPublicContent() {
    try {
      ...
      // localStorage blocked (e.g. file:// in some browsers) — fall through to fetch
    }

    return fetch('content.json', { cache: 'no-store' })
      .then((res) => {
        if (!res.ok) throw new Error('content.json not found (' + res.status + ')');
        return res.json();
      })
      .then((data) => deepMerge(deepClone(DEFAULT_CONTENT), data))
      .catch((e) => {
        console.warn('Store: could not load content.json, using defaults.', e);
        return deepClone(DEFAULT_CONTENT);
      });
  }
